...
$select = "SELECT emp.*,company.*,post.image,post.postID,post.lookingfor,post.jobdesc,post.jobtype,post.startdate AS Pstartdate,post.enddate AS Penddate,post.street,post.city,post.province,post.zipcode,post.createdat,post.salary,post.rate,stu.userID AS stuId,stu.firstname as stuFname,stu.lastname AS stuLname,stu.midname AS stuMname,address.street AS Sstreet,address.city AS Scity,address.province AS Sprovince,address.zipcode AS Szipcode,stu.contactno AS Scontactno,user.email,apply.*,applicant.applicantID,applicant.applicantID,applicant.status AS Astatus,applicant.`date@`,schedule.* 
FROM apply 
...
WHERE post.postID = '$post' AND stu.userID = '$id'";
