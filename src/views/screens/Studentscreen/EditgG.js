...
const EditG = ({navigation, error, route, onFocus=()=>{}, ...props
    ...
    const {gname,gaddress,gcontactno} =route.params
...
if (!inputs.gaddress){
  ...
  const register = () => {
    ...
    setTimeout(() => {
      setLoading(false);
axiosRequest.post('/api/editguardian.php', JSON.stringify(Data), headers)  
      .then((response) => {      
        console.log(response.data)
        if(response.data === "Guardian info Updated Successfully!!"){
